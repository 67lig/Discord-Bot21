...
// ── Keep-alive ping (autoscale — prevents idle sleep) ─────────────────────────
// Pings the app's own public URL every 4 minutes so autoscale never idles out.
const PING_INTERVAL_MS = 2 * 60 * 1000;

function startKeepAlive(publicUrl: string) {
  setInterval(async () => {
    try {
  ...
  logger.info({ publicUrl, intervalMs: PING_INTERVAL_MS }, "Keep-alive started");
}
...
app.listen(port, (err) => {
  ...
  // Start keep-alive in production using the public domain
  const replitDomain = process.env["REPLIT_DOMAINS"];
  if (replitDomain) {
    const publicUrl = `https://${replitDomain.split(",")[0]!.trim()}`;
